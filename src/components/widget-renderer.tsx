...
export const WidgetRendererProps = z.object({
  title: z
    .string()
    .describe("Short title for the visualization, e.g. 'Character Map' or 'Story Timeline'"),
  description: z
    .string()
...
// ─── Injected CSS: SVG Pre-Built Classes ────────────────────────────
...
// ─── Injected CSS: Form Element Styles ──────────────────────────────
...
// ─── Injected JS: Bridge Functions + Auto-Resize ────────────────────
const BRIDGE_JS = `
// Bridge: sendPrompt
...
export function WidgetRenderer({ title, description, html }: WidgetRendererProps) {
  ...
  // Write to iframe imperatively to avoid redundant reloads
  useEffect(() => {
  ...
  // Fallback: force-show after 4s if iframe hasn't reported ready
  useEffect(() => {
